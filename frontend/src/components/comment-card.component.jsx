...
const CommentCard = ({ index, leftVal, commentData }) => {
  ...
  const handleReply = () => {
    if (!access_token) {
      return toast.error("Kindly login to add a reply");
    }
  ...
  const LoadMoreRepliesBtn = () => {
    const parentIndex = getParentIndex();
    let len = index - parentIndex;

    let loadbtn = (
      <button
        onClick={() =>
          showReplies({ skip: index - parentIndex, currentIndex: parentIndex })
        }
    ...
    if (commentsArr[index + 1]) {
      if (
        ...
        if (len < commentsArr[parentIndex].children.length) {
          return loadbtn;
        }
      }
      ...
      <div className="my-5 p-6 rounded-md border border-grey">
        ...
        <div className="flex gap-5 items-center mt-5">
          ...
          (username === commented_by_username || username === blog_author) ? (
            <button
              className="py-2 px-3 rounded-md ml-auto hover:bg-red/30 hover:text-red flex items-center"
